...
show_usage() {
  ...
  printf "  %s --dry-run                            (preview actions without executing)\n" "$0"
}
...
# Helper: print or execute a command depending on dry-run mode
run_cmd() {
  if [ "$DRY_RUN" = "1" ]; then
...
# Helper: show prompt message in dry-run mode
dry_run_prompt() {
  if [ "$DRY_RUN" = "1" ]; then
    printf "%b[DRY-RUN] Would prompt: %s%b\n" "$YELLOW" "$1" "$RESET"
  fi
}
...
do_release() {
  ...
  if [ "$CURRENT_BRANCH" != "$DEFAULT_BRANCH" ]; then
    ...
    if [ "$DRY_RUN" != "1" ]; then
      ...
      dry_run_prompt "Proceed with release from '$CURRENT_BRANCH'?"
    fi
  fi
  ...
  if [ "$LOCAL" != "$REMOTE" ]; then
    if [ "$LOCAL" = "$BASE" ]; then
        ...
        if [ "$DRY_RUN" != "1" ]; then
          ...
          dry_run_prompt "Push changes to remote before releasing?"
        fi
  ...
  if git rev-parse "$TAG" >/dev/null 2>&1; then
    ...
    if [ "$DRY_RUN" != "1" ]; then
      ...
      dry_run_prompt "Tag exists. Skip tag creation and proceed to push?"
    fi
  ...
  if [ -z "$SKIP_TAG_CREATE" ]; then
    ...
    if git config --get user.signingkey >/dev/null 2>&1 || [ "$(git config --get commit.gpgsign)" = "true" ]; then
      ...
      run_cmd git tag -s "$TAG" -m "Release $TAG"
    else
  ...
  if [ "$DRY_RUN" != "1" ]; then
    ...
    dry_run_prompt "Push tag '$TAG' to remote?"
  fi
  ...
  if [ "$DRY_RUN" = "1" ]; then
    printf "%b[DRY-RUN] Tag '%s' would be pushed to remote%b\n" "$YELLOW" "$TAG" "$RESET"
    printf "%b[DRY-RUN] Pushing tag would trigger the release workflow%b\n" "$YELLOW" "$RESET"
    printf "\n%b[DRY-RUN] Dry run complete. No changes were made.%b\n" "$YELLOW" "$RESET"
  else
